	...
	$.each(acitylist, function(aIndex) {
		if (favcityID == acitylist[aIndex][2])
			tHtml += "<div class='cityline' id='citem_" + aIndex + "' cturn='"
					+ acitylist[aIndex][2] + "'><span class='ralign'><b>"
					+ acitylist[aIndex][1] + "</b></span></div>\n";
		else
			tHtml += "<div class='cityline' id='citem_" + aIndex + "' cturn='"
					+ acitylist[aIndex][2] + "'><span class='ralign'>"
	...
	if (sugSelectItem != atarget) {
		...
		if (atarget != 0) {
			try {
				...
				citySelected = fav_cities[Number(city_j.attr('cturn'))];
...
function showAllCity() {
	...
	tHtml = '<div class="com_hotresults" id="thetable" style="width:440px">'
			+ '<div style="width:100%;">'
			+ '<div class="ac_title">'
			+ '<span>'
			+ ''
			...
			+ '<li class="action" index="1" method="liHotTab"  onclick="js(1)" id="nav_list1">全部</li>'
			+ '<li index="2" method="liHotTab"  onclick="js(2)" id="nav_list2">总裁级</li>'
			+ '<li index="3" method="liHotTab"  onclick="js(3)" id="nav_list3">总经理级</li>'
			+ '<li index="4" method="liHotTab"  onclick="js(4)" id="nav_list4">经理级</li>'
			+ '<li index="5" method="liHotTab"  onclick="js(5)" id="nav_list5">招聘专员</li>'
			+ '<li index="6" method="liHotTab"  onclick="js(6)" id="nav_list6">人力助理</li>'
			+ '</ul>'
...
function filterCity(aKeyword) {
	...
	for ( var i = 0; i < fav_cities.length; i++) {
		if (isMatchCity(fav_cities[i], aKeyword, isPinyin))
			aList.push(fav_cities[i]);
	}
	if (aList.length > 0) {
		$("#top_cities")
...
function isMatchCity(aCityInfo, aKey, aisPinyin) {
	...
	var aInfo = [ aCityInfo[2].toLowerCase(), aCityInfo[1],
			aCityInfo[0].toLowerCase() ];
